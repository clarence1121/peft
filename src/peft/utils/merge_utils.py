...
def sce_weight(task_tensors: torch.Tensor) -> torch.Tensor:
    """
    Compute normalized weights (coefficients) for each task based on squared magnitude (energy).
    """
    # Calculate mean of squared elements for each task vector (shape: [T] if task_tensors shape is [T, ...])
    weights = torch.mean(task_tensors**2, dim=tuple(range(1, task_tensors.dim())))
    weight_sum = torch.sum(weights).item()
    if abs(weight_sum) < 1e-6:
        # If all energies are zero (unlikely), use uniform weights
        return torch.ones_like(weights) / weights.shape[0]
    return weights / weight_sum

def sce_mask(task_tensors: torch.Tensor, density: float, mask_dtype: Optional[torch.dtype] = None) -> torch.Tensor:
    """
    Select top-(density)% variance elements across tasks. Returns a mask of the same shape as one task tensor.
    """
    if mask_dtype is None:
        mask_dtype = task_tensors.dtype
    # Edge cases: density 0 -> mask out everything; density >=1 -> keep everything
    if density <= 0:
        return torch.zeros_like(task_tensors[0], dtype=mask_dtype)
    if density >= 1:
        return torch.ones_like(task_tensors[0], dtype=mask_dtype)
    # Compute variance across the task dimension for each parameter position
    var = torch.var(task_tensors, dim=0, unbiased=False)
    # Consider only parameters with non-zero variance
    nonzero_count = torch.count_nonzero(var).item()
    k = int(nonzero_count * density)
    if k <= 0:
        return torch.zeros_like(task_tensors[0], dtype=mask_dtype)
    # Select indices of the top-k variance values
    flat_var = var.view(-1)
    _, topk_idx = torch.topk(flat_var.abs(), k=k, largest=True)
    mask = torch.zeros_like(flat_var, dtype=mask_dtype)
    mask[topk_idx] = 1
...
def sce(
    ...
    """
    Merge multiple task tensors using SCE (Select-Coefficient-Erase).
    """
    # Stack task tensors into shape [T, ...] for vectorized operations
    task_stack = torch.stack(task_tensors, dim=0)  # shape: (T, *param_shape)
    # 1. Select: create binary mask of parameters to keep based on top variance
    mask = sce_mask(task_stack, density, mask_dtype=task_stack.dtype)
    # Apply mask to each task tensor (broadcasting across task dimension)
    task_stack_masked = task_stack * mask
    # 2. Coefficient: compute merging weight for each task (based on energy)
    task_weights = sce_weight(task_stack_masked)
    # 3. Majority sign (Erase minority): 
    # Determine majority sign for each parameter (between tasks) using the specified method.
    if majority_sign_method == "frequency":
        sign = torch.sign(task_stack_masked)
        pos_count = (sign > 0).sum(dim=0)
        neg_count = (sign < 0).sum(dim=0)
        majority_sign = torch.where(pos_count > neg_count, 1.0, torch.where(neg_count > pos_count, -1.0, 0.0))
    else:  # "total" method
        total_sum = (task_stack_masked * task_weights.view(-1, *([1] * (task_stack_masked.ndim - 1)))).sum(dim=0)
        majority_sign = torch.sign(total_sum)
    majority_sign = majority_sign.to(task_stack_masked.dtype)
    # 4. Erase minority: zero out contributions that do not match the majority sign
    sign = torch.sign(task_stack_masked)
    # Build masks for each parameter position
    no_majority_mask = (majority_sign == 0)
    pos_majority_mask = (majority_sign > 0)
    neg_majority_mask = (majority_sign < 0)
    # For each task, zero out contributions that do not match the majority sign
    for t in range(task_stack_masked.shape[0]):
        # Zero out where no majority
        task_stack_masked[t][no_majority_mask] = 0
        # Zero out negative contributions where positive majority
        task_stack_masked[t][pos_majority_mask & (sign[t] < 0)] = 0
        # Zero out positive contributions where negative majority
        task_stack_masked[t][neg_majority_mask & (sign[t] > 0)] = 0
    # 5. Merge: sum weighted contributions from all tasks for the selected parameters
    merged_param = (task_stack_masked * task_weights.view(-1, *([1] * (task_stack_masked.ndim - 1)))).sum(dim=0)
    return merged_param
